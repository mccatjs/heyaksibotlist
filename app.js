  ...
  const cookieParser = require('cookie-parser');//https://www.itemsatis.com/profil/377973/furylemons.html
...
app.get("/sitemap.xml", async function(req,res) {
  ...
  //https://www.itemsatis.com/profil/377973/furylemons.html
  const renderTemplate = (res, req, template, data = {}) => {
   ...
   app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        req.session.destroy(() => {
        res.json({ login: false, message: "You have been blocked from vCodes.", logout: true })//https://www.itemsatis.com/profil/377973/furylemons.html
        req.logout();
        });
        } else {
            try {
